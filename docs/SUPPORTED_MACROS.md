...
**중세국어(옛한글)도 된다.** 조합형 자모(U+1100–11FF, 확장 A/B)·방점(U+302E–302F)·
한자를 번들 명조가 모두 갖고 있고, OpenType 조합 기능(`ljmo`/`vjmo`/`tjmo`)이 살아 있어
`ᄊ`+`ㆍ` 가 한 음절로 합쳐진다. iOS 시스템 폰트에는 옛자모가 **하나도 없어서**(실측)
이 번들이 없으면 그릴 방법이 아예 없다.

```latex
\text{나\u{302E}랏\u{302E}말\u{302F}ᄊᆞ미 中듀ᇰ國귁}
```
